...
async fn can_serialize_unit_value() {
    ...
    // Set up conductors
    let mut conductor: SweetConductor =
        SweetConductor::from_config(ConductorConfig::default()).await;
    let app = conductor.setup_app("demo", &[dna]).await.unwrap();
    let (alice,) = app.into_tuple();

    // Alice gets a u32 response
    let value: u32 = conductor.call(&alice.zome("posts"), "get_u32", ()).await;

    assert_eq!(value, 1);

    // Alice gets a usize response
    let value: usize = conductor.call(&alice.zome("posts"), "get_usize", ()).await;
